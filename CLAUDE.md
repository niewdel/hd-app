...
### Proposal Builder — Line Items (consolidated 2026-04-24)
- The builder used to have three separate section cards (Pavement / Site Work / Additional Items) with four breakdown tables underneath. That was consolidated into **one Line Items card** with three action buttons (Library / Preset / + Add Item) and **one Line Item Breakdown** table that groups rows by division with per-division subtotals and a Grand Total.
- Internal arrays are unchanged — `sections`, `pavementItems`, `siteWorkItems`, `extraItems`, `concItems` still exist and are still persisted individually to `snap` (no data-model migration was done; only the UI was unified). Each array's render function writes to its own sub-container (`#sections`, `#conc-rows`, `#pavement-items`, `#sitework-rows`, `#extra-rows`) which all live inside the single Line Items card.
- `+ Add Item` calls `addCustomItem()` which pushes a new extraItem with `isCustom:true`. The isCustom rows render a simplified form (name, description, qty, unit free-text, unit_price, total) with **no autocomplete**; `calcItemBid(item)` returns `(qty||0) * (unit_price||0)` for isCustom items — no markup / crew / trucking math. User bakes their own margin into unit_price.
- `openLibraryPicker()` is now called with no target argument. The picker shows all library categories and dispatches on the item's own category: asphalt/stone → compound section (via `addSection`); concrete → `pavementItems`; sitework categories → `siteWorkItems`; everything else → `extraItems`.
- `showPresetPicker()` is called with no target argument — always creates a pavement section via `addSection(p.name, p.layers)`.
- Breakdown rendering: `_renderGroupedBreakdown(rows, grand)` replaces four separate tbody populations. Division order: Asphalt → Stone → Concrete → Grading → Utility → Erosion Control → Striping → Signage → Clearing → Landscaping → Other. Helpers: `_divisionForTrade(trade)`, `_divisionForItem(item)`, `getLayerTrade(layer)`.
- **`BID_LIBRARY` is the single source of truth** — loaded from `hd_settings.hd_bid_library` in Supabase (41 items as of 2026-04-24). The hardcoded `DEFAULT_BID_LIBRARY` JS constant was deleted in the same sweep so autocomplete/lookup flows never surface deleted items.
