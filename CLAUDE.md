...
- `_send_lead_email(lead)` / `_send_applicant_email(applicant)` — Gmail OAuth (`GMAIL_TOKEN_JSON`), send from `admin@hdgrading.com`. Both send `multipart/alternative` (HTML + plain-text fallback).
